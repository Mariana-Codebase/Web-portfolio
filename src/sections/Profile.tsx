...
export const Profile: React.FC<ProfileProps> = ({ themeColors }) => {
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 animate-in slide-in-from-bottom-5 duration-700">
      <div className={`${themeColors.cardBg} border ${themeColors.cardBorder} p-8 md:p-10 rounded-[3rem] lg:col-span-6 h-full flex flex-col justify-center transition-all duration-1000`}>
        <h3 className="text-blue-600 font-mono text-[11px] tracking-[0.25em] mb-6 uppercase italic font-black">{t.profileTitle}</h3>
        <p className="text-3xl md:text-4xl font-black italic tracking-tighter uppercase mb-6 leading-none">{t.profileSlogan}</p>
        <p className={`text-base md:text-lg ${themeColors.textSec} leading-relaxed font-medium`}>{t.profileDesc}</p>
      </div>
      <div className="lg:col-span-6 flex flex-col gap-5">
        ...
        <div className={`${themeColors.cardBg} border ${themeColors.cardBorder} p-6 md:p-8 rounded-[3rem]`}>
          ...
          <div className="space-y-5">
            ...
            <div className={`p-4 rounded-2xl border ${themeColors.cardBorder} ${isDarkMode ? 'bg-white/[0.02]' : 'bg-transparent'}`}>
              <h5 className="text-[11px] font-black tracking-widest text-blue-500/90 mb-3 uppercase italic">{t.stackCategories.monitoring}</h5>
              <div className="flex flex-wrap gap-2">
                {DATA.skills.monitoring.map((s) => (
                  <span key={s} className={`px-3 py-1.5 ${isDarkMode ? 'bg-white/5' : 'bg-stone-200/70'} rounded-full text-[11px] font-black border ${themeColors.cardBorder} hover:bg-blue-600 hover:text-white transition-all tracking-tight uppercase`}>
                    {s}
                  </span>
                ))}
            ...
            <div className={`p-4 rounded-2xl border ${themeColors.cardBorder} ${isDarkMode ? 'bg-white/[0.02]' : 'bg-transparent'}`}>
              <h5 className="text-[11px] font-black tracking-widest text-blue-500/90 mb-3 uppercase italic">{t.stackCategories.development}</h5>
              <div className="flex flex-wrap gap-2">
                {DATA.skills.development.map((s) => (
                  <span key={s} className={`px-3 py-1.5 ${isDarkMode ? 'bg-white/5' : 'bg-stone-200/70'} rounded-full text-[11px] font-black border ${themeColors.cardBorder} hover:bg-blue-600 hover:text-white transition-all tracking-tight uppercase`}>
                    {s}
                  </span>
                ))}
