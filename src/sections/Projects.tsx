...
export const Projects: React.FC<ProjectsProps> = ({ themeColors, projectFilter, setProjectFilter }) => {
  ...
  const projectsToRender: ProjectItem[] = hasGithubData ? githubProjects : filteredProjects;
  const filters: Array<keyof typeof t.categories> = hasGithubData
    ? ['ALL']
  ...
  }, [hasGithubData, projectFilter, setProjectFilter]);
    ...
    <div className="space-y-10 animate-in fade-in duration-700">
      <div className="flex flex-col md:flex-row justify-between items-end gap-6">
        ...
        <div className={`flex gap-2 ${isDarkMode ? 'bg-white/5' : 'bg-white/80 shadow-md'} p-1.5 rounded-2xl border ${isDarkMode ? themeColors.cardBorder : 'border-stone-400/60'}`}>
          {filters.map(cat => (
            <button key={cat} onClick={() => setProjectFilter(cat)} className={`px-4 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${projectFilter === cat ? 'bg-blue-600 text-white shadow-lg' : isDarkMode ? 'text-neutral-400 hover:text-white hover:bg-white/5' : 'text-stone-700 hover:text-stone-900 hover:bg-stone-200/80'}`}>{t.categories[cat as keyof typeof t.categories]}</button>
          ))}
        </div>
      </div>
          ...
          <div key={i} className={`group ${themeColors.cardBg} border ${themeColors.cardBorder} p-8 rounded-[2.5rem] hover:border-blue-600 transition-all duration-500 min-h-[360px] flex flex-col justify-between`}>
            <div>
              <span className="text-[11px] font-black text-blue-600 mb-6 block tracking-[0.2em] uppercase italic">
                // {isGithubProject(p) ? 'GITHUB' : t.categories[p.c]}
              </span>
              ...
              <span className={`inline-flex px-3 py-1.5 rounded-lg text-[10px] font-black uppercase tracking-wider border ${
                isDarkMode
                  ? 'bg-white/5 border-white/10 text-neutral-300'
                  : 'bg-stone-100 border-stone-300 text-stone-700'
              }`}>
                {isGithubProject(p)
                  ? (p.language || (language === 'es' ? 'Sin lenguaje' : 'No language'))
                  : (p.tags?.[0] || (language === 'es' ? 'Sin lenguaje' : 'No language'))}
              </span>
